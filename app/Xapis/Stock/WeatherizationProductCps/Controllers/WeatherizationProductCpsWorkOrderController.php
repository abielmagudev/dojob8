...
class WeatherizationProductCpsWorkOrderController extends Controller
{
    private function save(Request $request, WorkOrder $work_order)
    {
        ...
        for($i = 0; $i < $products_count; $i++)
        {
            array_push($data, [
                ...
                'indications' => $request->indications[$i] ?? null,
    ...
    {        
        return view('WeatherizationProductCps/views/work-orders/show', [
            'extension' => $extension,
            'work_order_products' => WeatherizationProductCpsWorkOrder::with('product')->whereWorkOrder($work_order->id)->get(),
        ]); 
    }
